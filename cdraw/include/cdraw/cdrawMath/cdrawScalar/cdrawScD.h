...
extern "C" {
	...
	bool scIsUnityApproxD(fp64_t const x);
	...
	bool scIsNonUnityApproxD(fp64_t const x);
	...
	/// <param name="x_den">Input denominator.</param>
	/// <returns>Quotient of inputs if denominator is non-zero, otherwise zero.</returns>
	fp64_t scDivModD(fp64_t* x_mod_out, fp64_t const x_num, fp64_t const x_den);
	...
	fp64_t scSinCosrD(fp64_t* sin_out, fp64_t* cos_out, fp64_t const x);
	...
	/// <param name="x">Angle in degrees.</param>
	/// <returns><paramref name="x"/></returns>
	fp64_t scSinCosdD(fp64_t* sin_out, fp64_t* cos_out, fp64_t const x);
